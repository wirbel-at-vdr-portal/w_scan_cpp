...
void PrintIni(std::vector<TChannel>& List) {
  ...
  std::string day   = std::to_string(0    + tm->tm_mday);
  while(month.size() < 2) month.insert(0, "0");
  while(day.size() < 2) day.insert(0, "0");
...
void PrintVLC(std::vector<TChannel>& List) {
  ...
  for(auto c:UniqueChannels(List)) {
     ...
     if (c.Source.find('S') == 0) {
        ...
        if (c.DelSys == 1) ss << "2";
        ss << "://frequency=" << freq_MHz * 1000 << "</location>" << std::endl;
        ...
        if (c.DelSys == 1) {
           if (c.StreamId  != 999)
              ss << INDENT << "<vlc:option>" << "dvb-plp-id=" << c.StreamId << std::endl;
           ss << INDENT << "<vlc:option>" << "dvb-modulation=";
	        if (c.Modulation == 2)       ss << "QPSK";
	        else if (c.Modulation == 5)  ss << "8PSK";
	        else if (c.Modulation == 6)  ss << "16APSK";
...
void PrintInitial(std::vector<TChannel>& List) {
  ...
  OutputLine("# https://gen2vdr.de/wirbel");
  OutputLine("#------------------------------------------------------------------------------");
